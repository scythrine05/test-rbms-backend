...
export const officerValidation = {
    createOfficer: z.object({
        body: z.object({
            ...
            if ((data.role === 'SENIOR_OFFICER' || data.role === 'JUNIOR_OFFICER' || data.role === 'USER') && !data.managerId) {
                return false; // Senior, Junior, and User must have a managerId
            }
            return true;
        }, {
            message: "Invalid role configuration. Please check adminId and managerId requirements for the selected role."
        })
    }),
