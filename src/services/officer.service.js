...
export const getOfficersUnderBranch = async (branchOfficerId, page = 1, limit = 10) => {
...
export const getOfficersUnderAdmin = async (adminId, page = 1, limit = 10) => {
    ...
    const [officers, total] = await Promise.all([
        prisma.user.findMany({
            where: {
                OR: [{ adminId, role: "BRANCH_OFFICER" }, { managerId: { in: branchOfficerIds } }],
            },
            select: {
                id: true,
                name: true,
    ...
    switch (creatorRole) {
        case "SUPER_ADMIN":
            if (data.role === "ADMIN") {
                ...
                if (!data.adminId)
                    throw new Error("Admin ID is required when creating a Branch Officer");
...
export const deleteOfficer = async (id, deleterId, deleterRole) => {
    ...
    const hasSubordinates = await prisma.user.findFirst({
        where: {
            OR: [{ managerId: id }, { adminId: id }],
        },
    });

    if (hasSubordinates) {
        throw new Error(
            "Cannot delete officer with subordinates. Reassign or delete subordinates first.",
        );
    }
